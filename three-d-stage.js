  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      const fit = () => {
        ...
        // setSize clears the drawing buffer — redraw at once so a capture
        // (or a paused rAF) never sees an empty canvas.
        if (this._object || scene.children.length) renderer.render(scene, camera);
      };
      fit();
    ...
    setObject(object) {
      ...
      this._sphere = box.isEmpty() ? null : box.getBoundingSphere(new THREE.Sphere());
      this._renderer.render(this._scene, this._camera);
      this._setButtonsEnabled(true);
    }

    /** Re-frame the camera from a direction vector [x,y,z] (object space),
     *  at distMul × the auto-fit distance. */
    frameFrom(dir, distMul = 1) {
      const THREE = this._THREE;
      if (!THREE || !this._sphere) return;
      const s = this._sphere;
      const dist =
        (s.radius / Math.tan((this._camera.fov * Math.PI) / 360)) * 1.35 * distMul;
      const v = new THREE.Vector3(dir[0], dir[1], dir[2]).normalize();
      this._camera.position.copy(s.center).add(v.multiplyScalar(dist));
      this._camera.near = Math.max(dist / 100, 0.01);
      this._camera.far = dist * 200;
      this._camera.updateProjectionMatrix();
      this._controls.target.copy(s.center);
      this._controls.autoRotate = false;
      this._controls.update();
      this._renderer.render(this._scene, this._camera);
    }

    /** Access for pages that need to tune the scene (background, lights). */
    get three() { return { THREE: this._THREE, scene: this._scene, camera: this._camera, renderer: this._renderer, controls: this._controls, key: this._key, ground: this._ground }; }
